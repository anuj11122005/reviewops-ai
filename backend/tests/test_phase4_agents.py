...
@pytest.mark.asyncio
async def test_test_generation_agent():
    mock_gateway = AsyncMock()
...
@pytest.mark.asyncio
async def test_reviewer_recommendation_agent():
...
async def test_deployment_agent(mock_mlflow_client_class):
    ...
    mock_client_instance.transition_model_version_stage.assert_called_with(
        name="BugPredictor", version="2", stage="Production", archive_existing_versions=True
    )
    ...
    # Test error handling
    mock_client_instance.get_latest_versions.side_effect = Exception("MLflow API failure")
    with pytest.raises(AgentExecutionError):
        await agent.execute(state)
