...
FEATURE_FUNCTION_COUNT = Histogram(
    "pr_feature_function_count", "Number of functions in PR files"
)
...
class MonitoringAgent:
    ...
    async def execute(self, state: dict[str, Any]) -> dict[str, Any]:
        ...
        logger.info(
            f"[MonitoringAgent] Starting execution for PR {pull_number} (hash: {input_hash})"
        )

        try:
            for _filename, file_features in features.items():
            ...
            raise AgentExecutionError("MonitoringAgent", pull_number, state.get("head_sha", "unknown_hash"), str(e)) from e
